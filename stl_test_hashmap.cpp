...
//声明如下方式
unordered_map<myNode,int,function<size_t( const myNode& p )> > map;
//方法3：模板定制
...
int main()
{
    ...
    unordered_map<myNode,int> myMap;
    ...
    // for (unordered_map<myNode,int,hash_name>::iterator it = myMap.begin(); it != myMap.end(); it++)
    // {
    ...
    typedef unordered_map<myNode,int>::iterator unorder_it;
    ...
    cout<<"bucker: "<<myMap.bucket_size(2)<<endl;
